  ...
  return (
    ...
    >
      ...
      <div className="relative border-b border-gold/20 bg-gradient-to-r from-gold/5 to-transparent p-6">
        {forfait.badge && (
          <span className="absolute right-4 top-4 bg-gold px-3 py-1 font-body text-xs uppercase tracking-[0.15em] text-white">
            {forfait.badge}
          </span>
        )}
          ...
          {forfait.soins.map((soin) => {
            ...
            return (
              <li key={soin.slug} className="flex items-center gap-2 font-body text-[13px] text-text-mid">
                ...
                <span>{soin.nom}</span>
                <span className="ml-auto text-xs text-text-muted-brand">{soin.duree} min</span>
              </li>
            )
          })}
        </ul>
      ...
      <div className="border-t border-gold/20 p-6">
        <div className="flex items-baseline justify-between">
          ...
          <span className="bg-primary-light px-3 py-1 font-body text-xs font-medium text-primary-brand">
            −{formatPrix(forfait.economie)}
          </span>
        </div>
        <Link
          href={`/prise-rdv?forfait=${forfait.slug}`}
          className="mt-4 flex w-full items-center justify-center bg-primary-brand py-3 font-body text-xs uppercase tracking-widest text-white transition-colors duration-300 hover:bg-primary-dark"
        >
          Réserver ce forfait
        </Link>
      </div>
